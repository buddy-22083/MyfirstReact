...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check validation อีกรอบเพื่อความชัวร์
    if (!isFormValid) {
      alert("กรุณากรอกข้อมูลให้ครบทุกช่องก่อนวิเคราะห์ครับ 😊");
      return;
    }
        ...
        <div className="pt-12 pb-8 text-center">
          <div className="relative inline-block mb-4">
            ...
            <div className="relative w-28 h-28 md:w-36 md:h-36 flex items-center justify-center overflow-hidden mx-auto transition-transform hover:scale-105 duration-500">
              <img src={logoUrl} alt="Logo" className="w-full h-full object-contain drop-shadow-xl" onError={(e) => { e.target.style.display='none'; }} />
            </div>
          </div>
          ...
          {step === 'input' && (
            <form onSubmit={handleSubmit} className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <button 
                  ...
                  className={`group w-full mt-4 rounded-2xl py-4 flex items-center justify-center gap-2 shadow-lg transition-all font-bold text-lg
                    ${isFormValid 
                ...
                {!isFormValid && <p className="text-center text-xs text-slate-400">กรุณากรอกข้อมูลให้ครบทุกช่อง</p>}
            </form>
          )}
